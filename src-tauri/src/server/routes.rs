...
pub fn create_router(state: SharedState) -> Router {
    let api = Router::new()
        ...
        .route(
            "/fs/write-file-atomic",
            post(handlers::fs::write_file_atomic),
        )
        ...
        .route(
            "/fs/find-related-wiki-pages",
            post(handlers::fs::find_related_wiki_pages),
        )
        .route("/fs/create-directory", post(handlers::fs::create_directory))
        .route("/fs/file-exists", post(handlers::fs::file_exists))
        .route(
            "/fs/get-file-modified-time",
            post(handlers::fs::get_file_modified_time),
        )
        .route("/fs/get-file-size", post(handlers::fs::get_file_size))
        .route("/fs/get-file-md5", post(handlers::fs::get_file_md5))
        .route(
            "/fs/read-file-as-base64",
            post(handlers::fs::read_file_as_base64),
        )
        .route(
            "/fs/get-executable-dir",
            get(handlers::fs::get_executable_dir),
        )
        ...
        .route(
            ...
            post(handlers::vector::vector_upsert_chunks),
        )
        .route(
            "/vector/search-chunks",
            post(handlers::vector::vector_search_chunks),
        )
        .route(
            "/vector/delete-page",
            post(handlers::vector::vector_delete_page),
        )
        .route(
            "/vector/count-chunks",
            get(handlers::vector::vector_count_chunks),
        )
        .route(
            "/vector/legacy-row-count",
            get(handlers::vector::vector_legacy_row_count),
        )
        .route(
            "/vector/drop-legacy",
            post(handlers::vector::vector_drop_legacy),
        )
        // project — 4 routes
        ...
        .route(
            "/project/open-folder",
            post(handlers::project::open_project_folder),
        )
        .route(
            "/project/open-file-location",
            post(handlers::project::open_file_location),
        )
        // backup — 3 routes
        ...
        .route(
            ...
            post(handlers::backup::import_backup_upload),
        )
        // extract — 4 routes
        .route(
            "/extract/pdf-images",
            post(handlers::extract::extract_pdf_images),
        )
        .route(
            "/extract/office-images",
            post(handlers::extract::extract_office_images),
        )
        .route(
            "/extract/save-pdf-images",
            post(handlers::extract::extract_and_save_pdf_images),
        )
        .route(
            "/extract/save-office-images",
            post(handlers::extract::extract_and_save_office_images),
        )
        // cli — 8 routes
        .route("/cli/claude-detect", get(handlers::cli::claude_cli_detect))
        .route(
            "/cli/claude-models",
            get(handlers::cli::claude_cli_list_models),
        )
        ...
        .route(
            "/cli/codex-models",
            get(handlers::cli::codex_cli_list_models),
        )
        ...
        .route(
            "/sync/start-watcher",
            post(handlers::sync::start_project_file_watcher),
        )
        .route(
            "/sync/stop-watcher",
            post(handlers::sync::stop_project_file_watcher),
        )
        ...
        .route(
            "/sync/ignore",
            post(handlers::sync::ignore_file_change_task),
        )
        // clip — 4 routes
        ...
        .route(
            "/clip/set-config",
            post(handlers::clip::set_clip_server_config),
        )
        ...
        .route(
            "/book-analysis/analyze",
            post(handlers::book_analysis::analyze_book),
        )
        .route(
            "/book-analysis/status",
            get(handlers::book_analysis::get_book_analysis_status),
        )
        .route(
            "/book-analysis/list",
            get(handlers::book_analysis::list_book_analyses),
        )
        .route(
            "/book-analysis/delete",
            post(handlers::book_analysis::delete_book_analysis),
        )
        .route(
            "/book-analysis/export",
            post(handlers::book_analysis::export_book_analysis),
        )
        // events — SSE
