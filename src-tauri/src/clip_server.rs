...
/// Daemon status: 0=starting, 1=running, 2=port_conflict, 3=error, 4=stopped
static DAEMON_STATUS: AtomicU8 = AtomicU8::new(0);
static CLIP_SERVER_ENABLED: AtomicBool = AtomicBool::new(true);
static CLIP_SERVER_PORT: AtomicU16 = AtomicU16::new(DEFAULT_PORT);
...
const STOP_WAIT_RETRIES: u32 = 20;
...
pub struct ClipServerRuntimeConfig {
    pub enabled: bool,
    pub port: u16,
    pub status: &'static str,
}

impl Default for ClipServerConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            port: DEFAULT_PORT,
...
pub fn read_clip_server_config_from_store(store_path: &Path) -> Option<ClipServerConfig> {
    let content = std::fs::read_to_string(store_path).ok()?;
    let json: serde_json::Value = serde_json::from_str(&content).ok()?;
    let config = json.get("clipServerConfig")?;
...
pub fn apply_clip_server_config(config: ClipServerConfig) -> Result<ClipServerRuntimeConfig, String> {
    ...
    if !config.enabled {
        CLIP_SERVER_ENABLED.store(false, Ordering::Relaxed);
        stop_clip_server();
        return Ok(get_runtime_config());
    }

    if SERVER_THREAD_RUNNING.load(Ordering::Relaxed)
        && (previous_port != config.port || STOP_REQUESTED.load(Ordering::Relaxed))
    {
        STOP_REQUESTED.store(true, Ordering::Relaxed);
        wait_for_server_stop();
    }

    CLIP_SERVER_ENABLED.store(true, Ordering::Relaxed);
    STOP_REQUESTED.store(false, Ordering::Relaxed);
    start_clip_server();
    Ok(get_runtime_config())
}

pub fn get_runtime_config() -> ClipServerRuntimeConfig {
    ClipServerRuntimeConfig {
        enabled: CLIP_SERVER_ENABLED.load(Ordering::Relaxed),
        port: CLIP_SERVER_PORT.load(Ordering::Relaxed),
        status: get_daemon_status(),
    }
}

pub fn stop_clip_server() {
    CLIP_SERVER_ENABLED.store(false, Ordering::Relaxed);
    STOP_REQUESTED.store(true, Ordering::Relaxed);
    if !SERVER_THREAD_RUNNING.load(Ordering::Relaxed) {
...
pub fn start_clip_server() {
    if !CLIP_SERVER_ENABLED.load(Ordering::Relaxed) {
        DAEMON_STATUS.store(4, Ordering::Relaxed);
        return;
    }
    if SERVER_THREAD_RUNNING.swap(true, Ordering::Relaxed) {
        return;
    }
    STOP_REQUESTED.store(false, Ordering::Relaxed);
    DAEMON_STATUS.store(0, Ordering::Relaxed);
    thread::spawn(|| {
        run_clip_server_loop();
        SERVER_THREAD_RUNNING.store(false, Ordering::Relaxed);
...
fn run_clip_server_loop() {
        ...
        loop {
            ...
            match (request.method(), url.as_str()) {
                ...
                (&Method::Get, "/project") => {
                    let path = CURRENT_PROJECT.lock().unwrap_or_else(|p| p.into_inner()).clone();
                ...
                (&Method::Get, "/projects") => {
                    let projects = ALL_PROJECTS.lock().unwrap_or_else(|p| p.into_inner()).clone();
                    let current = CURRENT_PROJECT.lock().unwrap_or_else(|p| p.into_inner()).clone();
                    ...
                    if request.as_reader().read_to_string(&mut body).is_ok() {
                        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&body) {
                            if let Some(arr) = parsed["projects"].as_array() {
                                let mut projects = ALL_PROJECTS.lock().unwrap_or_else(|p| p.into_inner());
                                projects.clear();
                                for item in arr {
                ...
                (&Method::Get, "/clips/pending") => {
                    let mut pending = PENDING_CLIPS.lock().unwrap_or_else(|p| p.into_inner());
...
fn handle_clip(body: &str) -> String {
    ...
    // 校验 projectPath 不包含路径遍历，且必须是绝对路径
    let project_path_buf = std::path::PathBuf::from(&project_path);
    if !project_path_buf.is_absolute() {
        return r#"{"ok":false,"error":"projectPath 必须是绝对路径"}"#.to_string();
    }
    // 拒绝包含 .. 的路径，防止路径遍历
    if project_path.contains("..") {
        return r#"{"ok":false,"error":"projectPath 不允许包含路径遍历符 .."}"#.to_string();
    }
