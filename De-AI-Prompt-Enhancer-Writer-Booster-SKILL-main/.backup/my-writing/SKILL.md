...
- **控制 AI 高频关联句式：** `一旦……就/只有……才/无论……都/随着……的/正是因为……所以/正是……才/通过……来` 全文合计 `<= 2`，且避免连续两段使用同类句式；超出即改为轻量条件句（要是/若/倘/靠/凭）或直接铺事实。

- **清零戏剧化揭露修辞：** 禁用「遮羞布/最后一块遮羞布/面具/画皮/伪装/外衣/皇帝的新衣/幌子/烟幕弹」的揭露比喻框架；禁用「扯下/撕下/揭下/摘下/掀开 + 遮蔽物」「剥开/戳穿/戳破/揭穿/撕开/揭开 + 表象/真面目/本质/真相/外壳/包装」句式（逐字引文除外），统一改为直接给事实与判断。

- **控制 AI 高频「极值判断」句式：** `最……的地方在于/是`、`真正……的是`、`……之处在于`、`更……的是` 这类极值判断框架全文合计 `<= 1`，超出即改为直接铺事实和判断；即使只出现 1 次也必须紧跟可核对事实。

- **清理 AI 伪学术腔高频词：** 禁用「舆论场/话语场/权力场/博弈场/公共场域/意义场域」等“XXX场/场域”抽象化合物；禁用「宏大叙事/国家叙事/官方叙事/主流叙事/元叙事」等“叙事”分析标签（文学批评语境除外）；禁用「摊开来看/摊开来说/把XXX摊开来/摊开来讲」；禁用非戏剧语境中的「XX初登台/正式登台/粉墨登场」；禁用「面子上是X，里子是Y/给足面子、掏空里子」这类二元套壳；并对「底层逻辑/顶层设计/认知升级/降维打击/赛道/红利/抓手/闭环/存量/增量/博弈（非博弈论语境）/范式/路径依赖（非经济学语境）/权柄/话语权（非学术引用）」执行密度控制：全文合计 `<= 1`。
