...
6. **严禁** - 严禁使用任何加粗符号；严禁使用"咱们"；严禁使用"房间里的大象"这种被AI用烂的比喻，同类禁止的 AI 高频隐喻词（比喻义）还包括「噪音/背景噪音、信号、滤镜、解药、光谱、底色、土壤、拼图、镜像、路径」；严禁使用"当……时"的句式；严禁使用AI伪口语化高频词「拆一拆、拆开来看、掰开、盘一盘、聊一聊、捋一捋、盘点一下、划重点、敲黑板」；严禁堆叠AI高频分析动词「拆解、梳理、剖析、解构、聚焦、洞察、深耕、赋能、助力、践行、驱动、构建、打造」（偶尔1次可接受，2次以上命中即改）；**严禁"分析师讲解"语姿与步骤承接过渡句**——凡是"把X做完/说完/看完/拆完/理完/弄清/厘清/讲完/聊完/分析完/搞清楚，再/便/就/回头/然后Y"这类句式一律改写（它暴露的是"我刚帮你完成了步骤A，现在进入步骤B"的讲课逻辑），禁止"先看A""接着看B""看完了A再说B""把A理清之后"等暗示引导读者完成思维步骤的句子，禁止在正文中交代分析流程（"上面说的是X，下面说Y"），改写策略是删掉步骤承接的前半句直接从新事实或判断起笔。这是逐词黑名单无法根治的深层AI痕迹——禁了"拆一拆"，LLM就输出"拆完"；禁了"拆完"就输出"看完/说完/理完"，必须从句式模板层面拦截。
...
- **去除AI高频词：** 严禁使用以下词汇：*"总而言之"、"综上所述"、"值得注意的是"、"双刃剑"、"多面性"、"织就"、"画卷"*。另严禁以下词汇的**比喻义**用法（字面义不受限）：*"噪音/背景噪音"、"信号"、"底色"、"光谱"、"滤镜"、"解药"、"土壤"、"拼图"、"镜像"、"路径"*。

- **去除AI伪口语化高频词：** 严禁使用以下"动词+一+动词"式的分析师口吻词汇：*"拆一拆"、"拆开来看"、"掰开"、"掰开来看"、"盘一盘"、"聊一聊"、"捋一捋"、"盘点一下"、"划重点"、"敲黑板"*。严禁堆叠以下AI高频分析动词（偶尔1次可接受，2次以上命中即改）：*"拆解"、"梳理"、"剖析"、"解构"、"聚焦"、"洞察"、"深耕"、"赋能"、"助力"、"践行"、"驱动"、"构建"、"打造"*。严禁以下降维引导语连续出现（全文合计≤1）：*"说白了"、"本质上（就是）"、"归根结底"、"一句话概括"、"简单来说"、"换个角度看"、"从某种意义上说"、"不妨这样理解"*。
...
> Gallery 825是 LAAA 的当代艺术展厅，四个独立空间加起来有三千多平方英尺。
...
- ✓ **步骤承接过渡句？** "把X拆完/说完/看完/理完，再/便/就Y"一律删掉前半句，直接从新事实起笔
